...
export default function Usertable({ onUserDeleted }) {
    ...
    const fetchData = async () => {
        try {
            const fetchuser = await axios.get('http://192.168.1.11:8000/auth/users');
    ...
    const handleDeleteConfirmed = async () => {
        try {
            await axios.delete(`http://192.168.1.11:8000/auth/users/${deleteId}`);
            setData(prevState => ({
                ...prevState,
  ...
  return (
    <div>
        ...
        <div className='px-4 md:px-10 overflow-x-auto w-full'>
        <table className="w-full border-2 border-blue min-w-[600px]">
            <thead>
